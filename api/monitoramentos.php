<?php
// api/monitoramentos.php — AJAX endpoints para o dashboard
...
switch ($acao) {
    ...
    // ---- Lista MVPs (globais, filtro opcional por busca) ----
    case 'listar_mvps':
        $sid = (int)($_GET['servidor_id'] ?? 0);
        // servidor_id ainda é recebido para compatibilidade mas MVPs são globais
        $stmt = $pdo->prepare(
        ...
        // Monta URL da imagem server-side
        foreach ($rows as &$r) {
    ...
    case 'iniciar':
        ...
        // Valida formato datetime (YYYY-MM-DD HH:MM:SS)
        ...
        // Verifica se já está monitorando este MVP neste servidor
        $chk = $pdo->prepare(
    ...
    // ---- Marca MVP como morto (registra hora + calcula spawn estimado) ----
    case 'matar':
        $mid = (int)($_POST['monitoramento_id'] ?? 0);
        if (!$mid) { echo json_encode(['erro' => 'ID inválido']); exit; }

        // Busca dados para calcular spawn
        $stmt = $pdo->prepare(
    ...
    // ---- Finaliza (remove do monitoramento ativo) ----
    case 'finalizar':
        $mid = (int)($_POST['monitoramento_id'] ?? 0);
        if (!$mid) { echo json_encode(['erro' => 'ID inválido']); exit; }
